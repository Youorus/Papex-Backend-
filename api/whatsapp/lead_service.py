...
def _update_existing_lead_appointment(lead, parsed_date, service_summary: Optional[str]) -> None:
    ...
    et on log l'événement STATUS_CHANGED pour déclencher les automations.
    """
    ...
    LeadEvent.log(
        lead=lead,
        event_code="LEAD_CREATED",  # Déclenche handle_lead_created → SMS + Email
        actor=None,
        data={
...
def create_lead_from_kemora(
    ...
    Nouveau client  → Lead créé + Client associé + LeadEvent(LEAD_CREATED)
                      → AutomationEngine → SMS + Email automatiques
    Client existant → appointment_date mis à jour + LeadEvent(LEAD_CREATED)
                      → AutomationEngine → SMS + Email automatiques
    ...
    try:
        ...
        if not effective_phone:
            logger.warning("Kemora — création ignorée : téléphone manquant")
            return None
        ...
        #   4. handle_lead_created → SMS + Email automatiques
        with transaction.atomic():
