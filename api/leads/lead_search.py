...
def _to_aware(value: Optional[object], *, end_of_day: bool = False) -> Optional[datetime]:
    ...
    if isinstance(value, date) and not isinstance(value, datetime):
        dt = datetime.combine(value, time(23, 59, 59, 999999) if end_of_day else time.min)
...
def _normalize_avec_sans(value: Optional[str]) -> Optional[str]:
    ...
    if value in {"avec", "oui", "with", "true", "1"}: return "avec"
    if value in {"sans", "non", "without", "false", "0"}: return "sans"
    return None
...
class LeadSearchView(APIView):
    ...
    def get(self, request):
        ...
        contracts_today = Contract.objects.filter(
            client__lead_id__in=Subquery(qs.values("id")),
